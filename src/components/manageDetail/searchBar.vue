...
<template>
    <el-row class="barContainer">
        ...
        <el-col :span="8">
            {{ model.groups[groupSelect].categories }}
        </el-col>
        <el-col :span="3">
            <div v-if="model.groups[groupSelect].status == 0"
            style="color: #dc5c5c">未面试</div>
            <div v-if="model.groups[groupSelect].status == 1"
            class="color: #ffbf00">已面试</div>
            <div v-if="model.groups[groupSelect].status == 2"
