...
const deleteGroup = (index: number) => {
  if (model.value.optionDetail.options.length == 1) {
    ...
    if (
      model.value.optionDetail.maxSelect >
      model.value.optionDetail.options.length
    ) {
      model.value.optionDetail.maxSelect =
        model.value.optionDetail.options.length;
      if (
        model.value.optionDetail.maxSelect < model.value.optionDetail.minSelect
      )
...
<template>
  <el-form-item>
    ...
    <div class="groupForm">
      <VueDraggableNext
        :list="model.optionDetail.options"
        :group="'select' + props.serial"
        animation="500"
        handle=".Sort"
      >
        <transition-group>
          <el-row
            class="selectOption"
            v-for="i in model.optionDetail.options.length"
            :key="i"
          >
            <el-col :span="19">
              <el-input
                :placeholder="'选项' + i"
                v-model="model.optionDetail.options[i - 1].label"
              />
            </el-col>
            <el-col :span="5" class="buttonGroup">
              <el-button
                ...
                @click="deleteGroup(i)"
              />
              <el-button class="Sort" type="primary" plain icon="Sort" />
            </el-col>
          </el-row>
        </transition-group>
      </VueDraggableNext>
      <el-button class="addButton" @click="addGroup()"> 新增选项 </el-button>
      <el-row class="numberInputContainer">
        <el-col :span="12">
          至少选<el-input-number
            class="numberInput"
            :min="1"
            :max="model.optionDetail.maxSelect"
        ...
        <el-col :span="12">
          至多选<el-input-number
            class="numberInput"
            :min="model.optionDetail.minSelect"
            :max="model.optionDetail.options.length"
